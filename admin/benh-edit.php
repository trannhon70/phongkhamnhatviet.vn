    ...
    <script>
        ...
        document.querySelector('button[name="submit"]').addEventListener('click', async function(event) {
            ...
            if (select.value !== "" && formData.name !== '') {
                ...
                try {
                    // First API endpoint
                    let response1 = await postData("https://phongkhamnhatviet.vn/api/benh/create-benh.php", formData);
                    if (response1.status === 'success') {
                        ...
                        // Second API endpoint
                        let response2 = await postData("https://namkhoa.phongkhamnhatviet.vn/api/benh/create-benh.php", formData);
                        if (response2.status === 'success') {
                            // toastr.success(response2.message);

                            // Third API endpoint
                            let response3 = await postData("https://haumontructrang.phongkhamnhatviet.vn/api/benh/create-benh.php", formData);
                            if (response3.status === 'success') {
                                // toastr.success(response3.message);

                                // Fourth API endpoint
                                let response4 = await postData("https://dalieu.phongkhamnhatviet.vn/api/benh/create-benh.php", formData);
                                if (response4.status === 'success') {
                                    // toastr.success(response4.message);

                                    // Fifth API endpoint
                                    let response5 = await postData("https://benhxahoi.phongkhamnhatviet.vn/api/benh/create-benh.php", formData);
